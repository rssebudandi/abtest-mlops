...
if __name__ == '__main__':
    ...
    threshold = 0.5
    print("Predicting")
    y_pred_xgbosst = model_xgboost.predict(X_test)
    mlflow.log_param("XGB_Predictions", y_pred_xgbosst)

    # evaluate model using cross validation
    scores = cross_val_score(model_xgboost, X_train, y_train, scoring='accuracy', cv=cv, n_jobs=-1)

    XGB__accuracy = accuracy_score(y_test, y_pred_xgbosst)

    print(XGB__accuracy)
    mlflow.log_metric("XGB_accuracy", XGB__accuracy)
    mlflow.sklearn.log_model(model_xgboost, "XGB Model")
